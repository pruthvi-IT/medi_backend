...
def fix_patients_schema(_auth=Depends(dev_auth)):
    ...
    BEGIN
      ...
      SELECT data_type INTO id_type FROM information_schema.columns WHERE table_name='patients' AND column_name='id';

      IF id_type IS NOT NULL AND id_type <> 'integer' THEN
        EXECUTE 'CREATE SEQUENCE IF NOT EXISTS patients_id_seq';
        EXECUTE 'ALTER TABLE patients ALTER COLUMN id DROP NOT NULL';
        EXECUTE 'ALTER TABLE patients ALTER COLUMN id TYPE INTEGER USING CASE WHEN (id ~ ''^[0-9]+$'') THEN CAST(id AS INTEGER) ELSE NULL END';
        EXECUTE 'UPDATE patients SET id = nextval(''patients_id_seq'') WHERE id IS NULL';
        EXECUTE 'ALTER TABLE patients ALTER COLUMN id SET NOT NULL';
      END IF;
      ...
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name='patients' AND column_name='id' AND column_default LIKE ''nextval%''
      ) THEN
